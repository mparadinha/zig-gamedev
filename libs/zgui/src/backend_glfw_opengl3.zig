...
pub fn init(window: *const anyopaque, glsl_version: ?[:0]const u8) void {
    if (!ImGui_ImplGlfw_InitForOpenGL(window, true)) {
        unreachable;
    }
    if (!ImGui_ImplOpenGL3_Init(if (glsl_version) |str| str.ptr else null)) {
        unreachable;
    }
}
...
// Those functions are defined in `imgui_impl_glfw.cpp` and 'imgui_impl_opengl3.cpp`
